...
namespace elevation_mapping {
  ...
  ElevationMapping::ElevationMapping(std::shared_ptr<rclcpp::Node>& nodeHandle) :
        ...
        robotPoseCacheSize_(200), // 姿势缓存大小
        // transformListener_(transformBuffer_),
        map_(nodeHandle),
        robotMotionMapUpdater_(nodeHandle),
        ignoreRobotMotionUpdates_(false),
        updatesEnabled_(true), // false 地图不会用新数据更新。                   
        maxNoUpdateDuration_(rclcpp::Duration::from_seconds(0.0)), // 最大不更新时间
        timeTolerance_(rclcpp::Duration::from_seconds(0.0)), // 允许传感器时间戳和系统时间之间存在的时间差
        fusedMapPublishTimerDuration_(rclcpp::Duration::from_seconds(0.0)), // 融合地图发布频率
        isContinuouslyFusing_(false), // Whether to continue to integrate
        visibilityCleanupTimerDuration_(rclcpp::Duration::from_seconds(0.0)), // 地图清理频率
        receivedFirstMatchingPointcloudAndPose_(false),  // 第一帧接收的标志位
        initializeElevationMap_(true),
        initializationMethod_(0), // 初始化模式。通常 0 代表平面初始化，即将机器人下方的区域默认设定为平整地面。
        lengthInXInitSubmap_(1.2), // 我们的地图 X 的长度
        lengthInYInitSubmap_(1.8), // 地图上 Y 的长度
        marginInitSubmap_(0.3),  // 地图边缘边距，防止机器人站在地图边缘
        initSubmapHeightOffset_(0.0) // 地图高度偏移
        {
  #ifndef NDEBUG
...
void ElevationMapping::setupSubscribers() {  // 处理已弃用的 point_cloud_topic 和 input_sources 配置。
  auto res = nodeHandle_->get_topic_names_and_types(); // 获取所有活跃的话题名和类型
  for (auto a:res){
...
bool ElevationMapping::readParameters() {
  ...
  nodeHandle_->declare_parameter("point_cloud_topic",""); // 声明一个空的点云话题，但是读取处被注释已经被弃置
  //FIXME: Fix for case when robot pose is not defined
  nodeHandle_->declare_parameter("robot_pose_with_covariance_topic", std::string("/pose")); // 包含协方差的机器人odom
  nodeHandle_->declare_parameter("track_point_frame_id", std::string("/robot")); //地图跟踪点坐标系
  nodeHandle_->declare_parameter("track_point_x", 0.0); // 地图的中心点，默认是机器人位置，会根据这个来移动地图
  nodeHandle_->declare_parameter("track_point_y", 0.0);
  nodeHandle_->declare_parameter("track_point_z", 0.0);
  nodeHandle_->declare_parameter("robot_pose_cache_size", 200); // 缓存机器人位姿消息的数量
  ...
  assert(robotPoseCacheSize_ >= 0); // 断言检查缓存必须大于0

  double minUpdateRate;
  nodeHandle_->declare_parameter("min_update_rate", 2.0); // 最小更新频率
  nodeHandle_->get_parameter("min_update_rate", minUpdateRate);
  if (minUpdateRate == 0.0) { // 如果最小频率为0打印警告
    ...
    maxNoUpdateDuration_ = rclcpp::Duration::from_seconds(1.0 / minUpdateRate); // hz转换成秒数->触发时间
  }
  assert(maxNoUpdateDuration_.seconds() != 0.0); // 断言

  double timeTolerance;
  nodeHandle_->declare_parameter("time_tolerance", 0.0); // 时间容差，对齐时间戳的误差
  ...
  nodeHandle_->get_parameter("fused_map_publishing_rate", fusedMapPublishingRate); // 融合地图的发布频率
  if (fusedMapPublishingRate == 0.0) {
    ...
    RCLCPP_WARN(nodeHandle_->get_logger(), 
        ...
        "called.");
  } else if (std::isinf(fusedMapPublishingRate)) { // 如果是无穷大
    isContinuouslyFusing_ = true; // 开启持续融合
    fusedMapPublishTimerDuration_ = rclcpp::Duration::from_seconds(0.0); // 定时器周期为0
  } else {
  ...
  nodeHandle_->get_parameter("visibility_cleanup_rate", visibilityCleanupRate); // 可视化清理周期
  if (visibilityCleanupRate == 0.0) {
    visibilityCleanupTimerDuration_ = rclcpp::Duration::from_seconds(0.0);
    RCLCPP_WARN(nodeHandle_->get_logger(), "Rate for visibility cleanup is zero and therefore disabled."); // 为0就禁止
  } else {
    visibilityCleanupTimerDuration_ = rclcpp::Duration::from_seconds(1.0 / visibilityCleanupRate);
    map_.visibilityCleanupDuration_ = 1.0 / visibilityCleanupRate; // 清理周期
  }
  ...
  nodeHandle_->get_parameter("map_frame_id", mapFrameId_); // 地图坐标系名称
  ...
  nodeHandle_->declare_parameter("length_in_x", 1.5); // 地图尺寸
  nodeHandle_->declare_parameter("length_in_y", 1.5);
  nodeHandle_->declare_parameter("position_x",  0.0); // 地图中心坐标
  nodeHandle_->declare_parameter("position_y",  0.0);
  nodeHandle_->declare_parameter("resolution",  0.01); // 地图分辨率
  ...
  nodeHandle_->declare_parameter("min_variance", pow(0.003, 2)); // 方差下限0.003^2
  nodeHandle_->declare_parameter("max_variance", pow(0.03, 2)); // 上限0.03^2
  nodeHandle_->declare_parameter("mahalanobis_distance_threshold", 2.5); // 马氏距离离群点阈值
  nodeHandle_->declare_parameter("multi_height_noise", pow(0.003, 2)); // 多高度下方差噪声
  nodeHandle_->declare_parameter("min_horizontal_variance", pow(resolution / 2.0, 2));  // two-sigma 水平方向的方差范围 2倍标准差估计
  nodeHandle_->declare_parameter("max_horizontal_variance", 0.5);
  nodeHandle_->declare_parameter("underlying_map_topic", std::string()); // 底层地图话题把已有地图当成基础图层
  nodeHandle_->declare_parameter("enable_visibility_cleanup", true); // 开启可视化清除
  nodeHandle_->declare_parameter("enable_continuous_cleanup", false); // 关闭持续清除，每次更新都先清理
  nodeHandle_->declare_parameter("scanning_duration", 1.0); // 扫描时间窗
  nodeHandle_->declare_parameter("masked_replace_service_mask_layer_name", std::string("mask")); // 服务器使用掩膜
  ...
  // Settings for initializing elevation map 仅在初始化是调用
  nodeHandle_->declare_parameter("initialize_elevation_map", false); // 是否在启动时初始化
  nodeHandle_->declare_parameter("initialization_method", 0); // 初始化方法
  nodeHandle_->declare_parameter("length_in_x_init_submap", 1.2); // 初始化子图尺寸
  nodeHandle_->declare_parameter("length_in_y_init_submap", 1.8);
  nodeHandle_->declare_parameter("margin_init_submap", 0.3); // 子图边缘安全边距
  nodeHandle_->declare_parameter("init_submap_height_offset", 0.0); // 初始化高度偏移
  nodeHandle_->declare_parameter("target_frame_init_submap", std::string("/footprint")); // 初始化子图参考系
